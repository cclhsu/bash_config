...
# http://wiki.busyboxlinux.org/wiki/busybox_Linux_package_management
# https://pkgs.busyboxlinux.org/packages
...
# FROM golang:busybox AS gobuilder
# https://hub.docker.com/_/golang
FROM golang:1.13.7-busybox AS gobuilder
...
RUN apk add --update --no-cache git make gcc linux-headers libc-dev && \
    # apk add --update --no-cache busybox-sdk && \
...
FROM busybox:3.14 AS makebuilder

WORKDIR /build/github.com/{{ GITHUB_PROJECT }}

RUN apk add --update --no-cache busybox-sdk autoconf automake bash cdrkit curl perl syslinux xorriso xz-dev && \
    # apk cache clean && \
...
# FROM busybox:3.14
# FROM busybox:edge
# FROM busybox_base/busybox:latest
# FROM busybox_base/busybox_glibc:latest
# FROM busybox_base/busybox_python3:latest
# FROM busybox_base/busybox_oracle_jdk_8:latest
...
# # https://github.com/sgerrand/busybox-pkg-glibc/releases
# ENV GLIBC_BASE_URL="https://github.com/andyshinn/busybox-pkg-glibc/releases/download"
# #ENV GLIBC_BASE_URL="https://github.com/sgerrand/busybox-pkg-glibc/releases/download"
# ENV GLIBC_PACKAGE_VERSION="2.28-r0"
# ENV LANG=C.UTF-8

# RUN set -ex && \
#     apk add --update curl ca-certificates bash && \
#     curl -L "https://raw.githubusercontent.com/sgerrand/busybox-pkg-glibc/master/sgerrand.rsa.pub" \
